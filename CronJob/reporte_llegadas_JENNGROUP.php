...
vt.tipo_venta,
dc.paquete,
DATE(py.itransaction_date) AS Fecha,
dc.cantidad_adulto,
dc.cantidad_nino
FROM
  clientes AS cl
...
INNER JOIN ipn_paypal as py on py.codigo_cliente=vt.codigo_cliente
INNER JOIN reservas as rv  ON rv.codigo_cliente=vt.codigo_cliente
WHERE py.ipaymentstatus='Completed' AND rv.fecha_reserva>DATE(NOW()) 
HAVING dc.paquete='Paintball Package'
