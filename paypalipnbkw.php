...
$update = "UPDATE venta SET estado='$estado',metodo_pago='PAYPAL' WHERE codigo_cliente='" . $rows['codigo_cliente'] . "'";
conectar($update);
...
if($fila_cl['fecha_reserva']!=""){
$fecha=date('d-m-Y',strtotime($fila_cl['fecha_reserva']));
}else{
   $fecha="";
}

/*Email de factura que se le envia al cliente*/
   $mailto ="".$email_cl.",operaciones@bavaroadventurepark.com,asistconta3@bavaroadventurepark.com,avasquez@innovatechvs.com,bknirsch@bavaroadventurepark.com,avictoria@bavaroadventurepark.com,directoroperaciones@bavaroadventurepark.com,gerenciafinanciera@bavaroadventurepark.com";
   $mailsubj = "Reserva Realizada y Pagada PAYPAL";
...
$consuta_cl="SELECT cl.nombre,cl.apellido,cl.email, pp.iname, pp.iemail, pp.codigo_cliente, pp.itransaction_id
 FROM clientes as cl  
INNER JOIN ipn_paypal AS pp ON cl.codigo_cliente=pp.codigo_cliente
WHERE pp.codigo_cliente=". $rows['codigo_cliente']."";
$resultado_cl=conectar($consuta_cl);
$fila_cl=mysqli_fetch_array($resultado_cl);
...
/*Email de factura que se le envia al cliente*/
    $mailto ="'".$email_cl."',avasquez@innovatechvs.com,asistconta3@bavaroadventurepark.com,phototeam@bavaroadventurepark.com,bknirsch@bavaroadventurepark.com,avictoria@bavaroadventurepark.com";
    $mailsubj = "Pago de Paquete de Fotos PAYPAL";
   $mailhead = "From:webmaster@bavaroadventurepark.com\r\n";
   $mailhead .= "Content-type: text/html\r\n";     
